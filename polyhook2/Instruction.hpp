...
namespace PLH
{
    ...
    public:
        ...
        Instruction(uint64_t address,
                    ...
                    Mode mode)
        {
            Init(address, displacement, displacementOffset, isRelative, isIndirect, bytes, mnemonic, opStr, false,
                 false, mode);
        }
                    ...
                    Mode mode)
        {
            ...
            Init(address, displacement, displacementOffset, isRelative, isIndirect, Arr, mnemonic, opStr, false, false,
                 mode);
        }
        ...
        * **/
        ...
        void setRelativeDisplacement(const int64_t displacement)
        {
            ...
            assert(
                static_cast<size_t>(m_dispOffset) + m_dispSize <= m_bytes.size() && m_dispSize <= sizeof(m_displacement.
                    Relative));
        ...
        void setAbsoluteDisplacement(const uint64_t displacement)
        {
            ...
            * copy of the bytes**/
            ...
            const auto dispSz = size() - getDisplacementOffset();
            if (static_cast<uint32_t>(getDisplacementOffset()) + dispSz > m_bytes.size() || dispSz > sizeof(
                m_displacement.Absolute))
            {
                PolyHook2DebugBreak();
                return;
            }
            ...
            assert(
                (static_cast<uint32_t>(getDisplacementOffset())) + dispSz <= m_bytes.size() && dispSz <= sizeof(
                    m_displacement.Absolute));
        ...
        {
            if (to < from)
                return static_cast<T>(0 - (from - to) - insSize);
            return static_cast<T>(to - (from + insSize));
        }
        ...
        bool startsWithDisplacement() const
        {
            ...
            return getOperandTypes().front() == OperandType::Displacement;
        }
    ...
    private:
                  ...
                  Mode mode)
        {
            ...
            m_immediate = 0;
            m_immediateSize = 0;
            m_register = ZYDIS_REGISTER_NONE;
        ...
        bool m_isIndirect;
        // Does this instruction get its destination via an indirect mem read (ff 25 ... jmp [jmp_dest]) (only filled for jmps / calls)
        ...
        // Does this instruction have the displacement fields filled (only rip/eip relative types are filled)
        bool m_hasImmediate; // Does this instruction have the immediate field filled?
        Displacement m_displacement;
        // Where an instruction points too (valid for jmp + call types, and RIP relative MEM types)
    ...
    static_assert(std::is_nothrow_move_constructible_v<Instruction>,
                  "PLH::Instruction should be noexcept move constructible");
    ...
    inline std::ostream& operator<<(std::ostream& os, const Instruction& obj)
    {
        ...
        for (std::size_t i = 0; i < obj.size(); i++)
            byteStream << std::hex << std::setfill('0') << std::setw(2) << static_cast<unsigned>(obj.getBytes()[i]) <<
                " ";
    ...
    inline uint16_t calcInstsSz(const insts_t& insts)
    {
        ...
        for (const auto& ins : insts)
            sz += static_cast<uint16_t>(ins.size());
        return sz;
    }
    ...
    template <typename T>
    std::ostream& printInsts(std::ostream& out, const T& container)
    {
    ...
    inline insts_t makex64PreferredJump(const uint64_t address, const uint64_t destination)
    {
     ...
     * the jmp should land.**/
    inline insts_t makex64MinimumJump(const uint64_t address, const uint64_t destination, const uint64_t destHolder)
    {
    ...
    inline insts_t makex86Jmp(const uint64_t address, const uint64_t destination)
    {
        Instruction::Displacement disp{0};
        ...
        return {Instruction(address, disp, 1, true, false, bytes, "jmp", int_to_hex(destination), Mode::x86)};
    }

    inline insts_t makeAgnosticJmp(const uint64_t address, const uint64_t destination)
    {
    ...
    inline insts_t makex64DestHolder(const uint64_t destination, const uint64_t destHolder)
    {
        std::vector<uint8_t> destBytes;
        destBytes.resize(8);
        memcpy(destBytes.data(), &destination, 8);
        return insts_t{
            Instruction(destHolder, Instruction::Displacement{0}, 0, false, false, destBytes, "dest holder", "",
                        Mode::x64)
        };
    }
}
