...
namespace PLH
{
    ...
    {
        return static_cast<FnCastTo>(fnToCast);
    }
...
#define _PTR_MAX_VALUE ((void*)0xFFF00000)
    ...
    inline int my_wide_stricmp(const wchar_t* a, const wchar_t* b)
    {
        ...
        do
        {
            ca = static_cast<wint_t>(*a++);
            cb = static_cast<wint_t>(*b++);
    ...
    {
        size_t n = 0;
        while (addr[n] == pat[n] || msk[n] == static_cast<uint8_t>('?'))
        {
            if (!msk[++n])
            {
                return true;
...
#define getByte( x )		(getBits(x[0]) << 4 | getBits(x[1]))
    ...
    uint64_t boundAlloc(uint64_t min, uint64_t max, uint64_t size, ULONG pageProtection = PAGE_EXECUTE_READWRITE);
    uint64_t boundAllocLegacy(uint64_t min, uint64_t max, uint64_t size, ULONG pageProtection = PAGE_EXECUTE_READWRITE);
    ...
    std::string int_to_hex(T i)
    {
        ...
        stream << "0x" << std::setfill('0') << std::setw(sizeof(T) * 2) << std::hex
            << static_cast<uint64_t>(i); // We cast to the highest possible int because uint8_t will be printed as char
