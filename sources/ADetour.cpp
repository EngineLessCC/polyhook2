...
namespace PLH
{
    ...
    bool Detour::followJmp(insts_t& functionInsts, const uint8_t curDepth)
    {
        // NOLINT(misc-no-recursion)
        if (functionInsts.empty())
        {
            PLH_LOG("Couldn't decompile instructions at followed jmp", ErrorLevel::WARN);
            return false;
        }

        if (curDepth >= m_maxDepth)
        {
            PLH_LOG("Prologue jmp resolution hit max depth, prologue too deep", ErrorLevel::WARN);
            return false;
        }
        ...
        {
            PLH_LOG("setting: Do NOT follow CALL on fnAddress", ErrorLevel::INFO);
            if (functionInsts.front().isCalling())
            {
                PLH_LOG("First assembly instruction is CALL", ErrorLevel::INFO);
                return true;
            }
        }
        ...
        {
            PLH_LOG("Branching instruction without displacement encountered", ErrorLevel::WARN);
            return false;
        }

        const uint64_t dest = functionInsts.front().getDestination();
    ...
    bool Detour::expandProlSelfJmps(insts_t& prol, const insts_t& func, uint64_t& minProlSz, uint64_t& roundProlSz)
    {
        ...
        for (size_t i = 0; i < prol.size(); i++)
        {
            ...
            if (!branchMap.contains(inst.getAddress()))
                continue;
    ...
    )
    {
        ...
        for (auto& inst : prologue)
        {
            ...
            const auto dispSzBits = static_cast<uint8_t>(inst.getDispSize()) * 8;
            // 2^(bitSz-1) give max val, and -1 because signed ex (int8_t [-128, 127] = [-2^7, 2^7 - 1]
            const auto maxInstDisp = static_cast<uint64_t>(std::pow(2, dispSzBits - 1) - 1.0);
            const auto absDelta = static_cast<uint64_t>(std::llabs(delta));
    ...
    bool Detour::unHook()
    {
        if (!m_hooked)
        {
            PLH_LOG("Detour unhook failed: no hook present", ErrorLevel::SEV);
            return false;
        }

        MemoryProtector prot(m_fnAddress, calcInstsSz(m_originalInsts), R | W | X, *this);
    ...
    bool Detour::reHook()
    {
        ...
        {
            PLH_LOG("hook size must not be larger than nop prologue offset", ErrorLevel::SEV);
            return false;
        }
    ...
    insts_t Detour::make_nops(uint64_t address, uint16_t size) const
    {
        ...
        static constexpr uint8_t max_nop_size = 9;
        ...
        const auto max_nop_count = size / max_nop_size;
        const auto remainder_nop_size = static_cast<uint8_t>(size % max_nop_size);
