...
PLH::EatHook::EatHook(const std::string& apiName, const std::wstring& moduleName, const char* fnCallback,
                      uint64_t* userOrigVar)
    : EatHook(apiName, moduleName, (uint64_t)fnCallback, userOrigVar)
{
}

PLH::EatHook::EatHook(const std::string& apiName, const std::wstring& moduleName, const uint64_t fnCallback,
                      uint64_t* userOrigVar)
    : EatHook(apiName, moduleName, nullptr, fnCallback, userOrigVar)
{
}

PLH::EatHook::EatHook(const std::string& apiName, const HMODULE moduleHandle, const char* fnCallback,
                      uint64_t* userOrigVar)
    : EatHook(apiName, moduleHandle, (uint64_t)fnCallback, userOrigVar)
{
}

PLH::EatHook::EatHook(const std::string& apiName, const HMODULE moduleHandle, const uint64_t fnCallback,
                      uint64_t* userOrigVar)
    : EatHook(apiName, L"", moduleHandle, fnCallback, userOrigVar)
{
}

PLH::EatHook::EatHook(std::string apiName, std::wstring moduleName, const HMODULE moduleHandle,
                      const uint64_t fnCallback, uint64_t* userOrigVar)
    : m_moduleName(std::move(moduleName))
      , m_apiName(std::move(apiName))
      , m_fnCallback(fnCallback)
      , m_origFunc(0)
      , m_userOrigVar(userOrigVar) // arbitrary, size is big enough but an overshoot
      , m_allocator(64, 64)
      , m_trampoline(0)
...
bool PLH::EatHook::hook()
{
    ...
    {
        m_trampoline = (uint64_t)m_allocator.allocate(m_moduleBase, calc_2gb_above(m_moduleBase));
        if (m_trampoline == 0)
        {
            PLH_LOG(
                "EAT hook offset is > 32bit's. Allocation of trampoline necessary and failed to find free page within range",
                ErrorLevel::INFO);
            return false;
        }

        MemoryProtector protector(m_trampoline, 64, R | W | X, *this, false);

        ZydisDisassembler::writeEncoding(makeAgnosticJmp(m_trampoline, m_fnCallback), *this);
        offset = static_cast<size_t>(m_trampoline - m_moduleBase);

        PLH_LOG("EAT hook offset is > 32bit's. Allocation of trampoline necessary", ErrorLevel::INFO);
    }

    // Just like IAT, EAT is by default a writeable section
    // any EAT entry must be an offset
    MemoryProtector prot((uint64_t)pExport, sizeof(uintptr_t), R | W, *this);
...
bool PLH::EatHook::unHook()
{
    ...
    if (!m_hooked)
    {
        PLH_LOG("EatHook unhook failed: no hook present", ErrorLevel::SEV);
        return false;
    }
    ...
    MemoryProtector prot((uint64_t)pExport, sizeof(uintptr_t), R | W, *this);
    *pExport = static_cast<uint32_t>(m_origFunc);
    m_hooked = false;
    *m_userOrigVar = NULL;
...
uint32_t* PLH::EatHook::FindEatFunction()
{
    ...
    if (!m_moduleBase)
    {
        PLH_LOG("EAT | Failed to module base", ErrorLevel::SEV);
        return nullptr;
    }
...
uint64_t PLH::EatHook::FindModule()
{
    ...
    PLH_LOG("EAT | Failed to automatically find module", ErrorLevel::SEV);
...
uint32_t* PLH::EatHook::FindEatFunctionInModule() const
{
    ...
    const auto* pDos = (IMAGE_DOS_HEADER*)m_moduleBase;
    const auto* pNT = RVA2VA(IMAGE_NT_HEADERS*, m_moduleBase, pDos->e_lfanew);
    ...
    {
        PLH_LOG("PEs without export tables are unsupported", ErrorLevel::SEV);
        return nullptr;
    }

    const auto* pExports = RVA2VA(IMAGE_EXPORT_DIRECTORY*, m_moduleBase,
    ...
    const auto* pAddressOfNames = RVA2VA(uint32_t*, m_moduleBase, pExports->AddressOfNames);
    const auto* pAddressOfNameOrdinals = RVA2VA(uint16_t*, m_moduleBase, pExports->AddressOfNameOrdinals);
    ...
    PLH_LOG("API not found before end of EAT", ErrorLevel::SEV);
    return nullptr;
}
