...
PLH::ZydisDisassembler::ZydisDisassembler(Mode mode) : m_decoder(new ZydisDecoder()), m_formatter(new ZydisFormatter())
{
    m_mode = mode;
    ...
    {
        PLH_LOG("Failed to initialize zydis decoder", ErrorLevel::SEV);
        return;
    }

    if (ZYAN_FAILED(ZydisFormatterInit(m_formatter, ZYDIS_FORMATTER_STYLE_INTEL)))
    {
        PLH_LOG("Failed to initialize zydis formatter", ErrorLevel::SEV);
        return;
    }
...
)
{
    ...
    size_t read = 0;
    auto* buf = new uint8_t[static_cast<uint32_t>(size)];
    ...
    bool endHit = false;
    while (ZYAN_SUCCESS(
        ZydisDecoderDecodeFull(m_decoder, buf + offset, static_cast<ZyanUSize>(read - offset), &insInfo,
            decoded_operands)))
    {
        ...
        for (int i = 0; i < insInfo.operand_count; i++)
        {
            ...
            if (op.type == ZYDIS_OPERAND_TYPE_MEMORY && op.mem.type == ZYDIS_MEMOP_TYPE_MEM && op.mem.disp.
                has_displacement && op.mem.base == ZYDIS_REGISTER_NONE && op.mem.segment != ZYDIS_REGISTER_DS && inst.
                isIndirect())
            {
...
bool PLH::ZydisDisassembler::getOpStr(ZydisDecodedInstruction* pInstruction,
                                      const ZydisDecodedOperand* decoded_operands, uint64_t addr,
                                      std::string* pOpStrOut)
{
    char buffer[256];
    if (ZYAN_SUCCESS(
        ZydisFormatterFormatInstruction(m_formatter, pInstruction, decoded_operands, pInstruction->operand_count, buffer
            , sizeof(buffer), addr, ZYAN_NULL)))
    {
...
void PLH::ZydisDisassembler::setDisplacementFields(Instruction& inst, const ZydisDecodedInstruction* zydisInst,
                                                   const ZydisDecodedOperand* operands) const
{
    inst.setBranching(zydisInst->meta.branch_type != ZYDIS_BRANCH_TYPE_NONE);
    inst.setCalling(zydisInst->mnemonic == ZYDIS_MNEMONIC_CALL);
        ...
        case ZYDIS_OPERAND_TYPE_MEMORY:
            {
                ...
                if (zydisInst->attributes & ZYDIS_ATTRIB_IS_RELATIVE)
                {
                    ...
                    inst.setDisplacementSize(static_cast<uint8_t>(zydisInst->raw.disp.size / 8));
                    inst.setRelativeDisplacement(operand->mem.disp.value);
                }

                if ((zydisInst->mnemonic == ZYDIS_MNEMONIC_JMP && inst.size() >= 2 && inst.getBytes().at(0) == 0xff &&
                        inst.getBytes().at(1) == 0x25) ||
                    (zydisInst->mnemonic == ZYDIS_MNEMONIC_CALL && inst.size() >= 2 && inst.getBytes().at(0) == 0xff &&
                        inst.getBytes().at(1) == 0x15) ||
                    (zydisInst->mnemonic == ZYDIS_MNEMONIC_CALL && inst.size() >= 3 && inst.getBytes().at(1) == 0xff &&
                        inst.getBytes().at(2) == 0x15) ||
                    (zydisInst->mnemonic == ZYDIS_MNEMONIC_JMP && inst.size() >= 3 && inst.getBytes().at(1) == 0xff &&
                        inst.getBytes().at(2) == 0x25)
                )
                {
        ...
        case ZYDIS_OPERAND_TYPE_IMMEDIATE:
            {
                ...
                if (!inst.hasDisplacement() && zydisInst->attributes & ZYDIS_ATTRIB_IS_RELATIVE)
                {
                    ...
                    inst.setDisplacementSize(static_cast<uint8_t>(zydisInst->raw.imm->size / 8));
