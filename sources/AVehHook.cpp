...
PLH::AVehHook::AVehHook()
{
    if (m_refCount.m_count == 0)
    {
        ...
        if (m_hHandler == nullptr)
        {
            PLH_LOG("Failed to add VEH", ErrorLevel::SEV);
        }
    }
...
PLH::AVehHook::~AVehHook()
{
    ...
    if (m_refCount.m_count == 0)
    {
        ...
        const ULONG status = RemoveVectoredExceptionHandler(m_hHandler);
        m_hHandler = nullptr;
        if (status == 0)
        {
            PLH_LOG("Failed to remove VEH", ErrorLevel::SEV);
        }
    }
}
...
LONG CALLBACK PLH::AVehHook::Handler(EXCEPTION_POINTERS* ExceptionInfo)
{
    ...
    const uint64_t ip = ExceptionInfo->ContextRecord->XIP;
